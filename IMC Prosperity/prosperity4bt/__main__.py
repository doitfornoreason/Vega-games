...
from prosperity4bt.data import discover_limits, has_day_data
from prosperity4bt.file_reader import FileReader, FileSystemReader, PackageResourcesReader
from prosperity4bt.models import BacktestMetrics, BacktestResult, TradeMatchingMode
...
def parse_algorithm(algorithm: Path) -> Any:
    sys.path.insert(0, str(algorithm.parent))
    pkg_dir = Path(__file__).parent
    sys.path.insert(0, str(pkg_dir))
    utils_dir = pkg_dir.parent / "utils"
    if utils_dir.is_dir():
...
def load_config(config_path: Path) -> dict[str, Any]:
    """Load a JSON config file with backtest settings."""
...
def version_callback(value: bool) -> None:
    if value:
        try:
            ver = metadata.version("prosperity4bt")
        except metadata.PackageNotFoundError:
            ver = "dev"
        print(f"prosperity4bt {ver}")
        sys.exit(0)
...
def cli(
    ...
    merge_pnl: Annotated[bool, Option("--merge-pnl/--no-merge-pnl", help="Merge profit and loss across days (default: on).")] = True,
    ...
    verbose: Annotated[bool, Option("--verbose", help="Show detailed order matching and conversion info.")] = False,
    limits_file: Annotated[Optional[Path], Option("--limits-file", help="Path to JSON file with position limits per product.", show_default=False, exists=True, file_okay=True, dir_okay=False, resolve_path=True)] = None,
    config: Annotated[Optional[Path], Option("--config", help="Path to JSON config file with backtest settings.", show_default=False, exists=True, file_okay=True, dir_okay=False, resolve_path=True)] = None,
    show_metrics: Annotated[bool, Option("--metrics", help="Show detailed performance metrics after backtesting.")] = False,
    visualizer_url: Annotated[Optional[str], Option("--visualizer-url", help="Custom visualizer URL (default: Prosperity 3 visualizer).", show_default=False)] = None,
    ...
    # Apply config file settings (CLI flags override config)
    if config is not None:
        cfg = load_config(config)
        if not merge_pnl and cfg.get("merge_pnl", False):
            merge_pnl = True
        if match_trades == TradeMatchingMode.all and "match_trades" in cfg:
            match_trades = TradeMatchingMode(cfg["match_trades"])
        if limits_file is None and "limits_file" in cfg:
            limits_file = Path(cfg["limits_file"]).resolve()
        if not show_metrics and cfg.get("metrics", False):
            show_metrics = True
        if not verbose and cfg.get("verbose", False):
            verbose = True
        if visualizer_url is None:
            visualizer_url = cfg.get("visualizer_url")
    ...
    # Discover position limits
    limits = discover_limits(file_reader, limits_file)
    if limits:
        print(f"Loaded position limits for {len(limits)} products")

    show_progress_bars = not no_progress and not print_output and not verbose
    ...
    if len(parsed_days) > 1:
        ...
        if show_metrics:
            ...
            for m in all_metrics:
                ...
                agg.total_volume += m.total_volume
                agg.orders_submitted += m.orders_submitted
                agg.orders_filled += m.orders_filled
                for p, v in m.pnl_by_product.items():
                    agg.pnl_by_product[p] = agg.pnl_by_product.get(p, 0) + v
                for p, v in m.trades_by_product.items():
                    agg.trades_by_product[p] = agg.trades_by_product.get(p, 0) + v
                for p, v in m.volume_by_product.items():
                    agg.volume_by_product[p] = agg.volume_by_product.get(p, 0) + v
            print("\n--- Aggregated Metrics ---")
            print(agg.summary())
    ...
    if vis and output_file is not None:
        open_visualizer(output_file, visualizer_url)
