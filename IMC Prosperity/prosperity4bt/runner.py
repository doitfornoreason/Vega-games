...
def compress_state_for_visualizer(state: TradingState, orders: dict, conversions, trader_data: str, sandbox_row: SandboxLogRow) -> None:
    """Serialize trading state into the compressed format expected by jmerle's visualizer.

    The visualizer parser extracts compressed data from lambdaLog by finding '[[' and
    parsing the JSON array from that point. Format:
    [CompressedState, CompressedOrders, conversions, traderData, logs]
    """
    # CompressedTradingState - 8 element array
    compressed_state = [
        state.timestamp,
        state.traderData or "",
        [[s, l.product, l.denomination] for s, l in state.listings.items()],
        {s: [od.buy_orders, od.sell_orders] for s, od in state.order_depths.items()},
        [[t.symbol, t.price, t.quantity, t.buyer or "", t.seller or "", t.timestamp]
    ...
    # CompressedOrders: [[symbol, price, quantity], ...]
    compressed_orders = []
    ...
    # Conversions value
    conv_value = conversions if isinstance(conversions, int) else 0

    # CompressedAlgorithmDataRow: [state, orders, conversions, traderData, logs]
    compressed_row = [compressed_state, compressed_orders, conv_value, trader_data or "", ""]

    # Append to lambda_log (parser looks for '[[' to find start)
    json_str = json.dumps(compressed_row, separators=(",", ":"))
    if sandbox_row.lambda_log:
        sandbox_row.lambda_log += "\n" + json_str
    else:
        sandbox_row.lambda_log = json_str
...
def run_backtest(
    ...
    for timestamp in timestamps_iterator:
        ...
        # Clear previous tick's trades so only this tick's trades are serialized
        state.own_trades = {}
        state.market_trades = {}

        match_orders(state, data, orders, result, trade_matching_mode, metrics, verbose)

        # Serialize compressed state for visualizer
        compress_state_for_visualizer(state, orders, conversions, trader_data, sandbox_row)
