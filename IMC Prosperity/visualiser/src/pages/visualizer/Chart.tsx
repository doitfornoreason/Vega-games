...
export function Chart({ title, options, series, min, max, dayBoundaries, onHover }: ChartProps): ReactNode {
  const colorScheme = useActualColorScheme();
  const downsamplingFactor = useStore(state => state.featureToggles.downsamplingFactor);
  ...
  const fullOptions = useMemo((): Highcharts.Options => {
    ...
    // Step 4: Downsampling — scale dataGrouping units by factor
    const f = downsamplingFactor;
    const groupingUnits: [string, number[]][] = [['second', [1 * f, 2 * f, 5 * f, 10 * f]]];
        ...
        events: {
          load() {
            Highcharts.addEvent(this.tooltip, 'headerFormatter', (e: any) => {
              ...
              // Step 7: Fire onHover callback when tooltip updates
              if (onHover) {
                onHover(timestamp);
              }
  ...
  }, [colorScheme, title, options, series, min, max, dayBoundaries, onHover, downsamplingFactor]);
