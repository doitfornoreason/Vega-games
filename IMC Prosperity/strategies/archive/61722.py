            ...
            if product in state.order_depths:
               ...
               # Best bid: highest buy price (buy_orders is dict {price: volume}, volume positive)
                ...
                if len(order_depth.buy_orders) != 0:
                    best_bid = max(order_depth.buy_orders.keys())
                    best_bid_vol = order_depth.buy_orders[best_bid]
                    market_bid = next((bid for bid in sorted(order_depth.buy_orders, reverse=True) if bid + 1 < fair_value), None)
                    market_bid_vol = order_depth.buy_orders[market_bid] if market_bid is not None else 0
                    if best_bid > fair_value:
                ...
                if len(order_depth.sell_orders) != 0:
                    best_ask = min(order_depth.sell_orders.keys())
                    best_ask_vol = order_depth.sell_orders[best_ask]
                    market_ask = next((ask for ask in sorted(order_depth.sell_orders) if ask - 1 > fair_value), None)
                    market_ask_vol = order_depth.sell_orders[market_ask] if market_ask is not None else 0
                    if best_ask < fair_value:
